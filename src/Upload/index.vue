    ...
    <ul class="v-form__upload-preview">
      <li v-for="(item, index) in value" :key="index" class="v-form__upload-preview-item">
        ...
        <div v-if="item.loading" class="v-form__upload-preview-mask">
          <template>
            <i v-if="item.loadingText === '上传失败'" class="van-icon van-icon-warning-o van-uploader__mask-icon"></i>
            <div v-else class="van-loading van-loading--circular van-uploader__loading">
        ...
        <!-- 删除按钮 -->
        <span v-if="!disabled" class="van-icon van-icon-clear van-uploader__preview-delete" @click="_removeFile(index)"></span>
      </li>
...
export default {
  ...
  methods: {
    ...
    _selectFile ({ target }) {
      this.files = Array.from(target.files).map((file, index) => ({ file, index: this.value.length + index }))
